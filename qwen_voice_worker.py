...
def synthesize(request):
    ...
    identity=json.dumps([signature,text,'qwen-0.6b-bf16-v1'],ensure_ascii=False)
    ...
    if model is None:
        model=Qwen3TTSModel.from_pretrained(str(runtime/'qwen3-tts-base'),device_map='cpu',dtype=torch.bfloat16,attn_implementation='eager',local_files_only=True)
    ...
    wavs,rate=model.generate_voice_clone(text=text,language='Korean',voice_clone_prompt=prompts[signature],non_streaming_mode=True,max_new_tokens=420)
